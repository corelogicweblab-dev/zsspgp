-- Fix: invalid input value for enum user_role: "information_office"
-- Happens when RLS compares get_user_role() to 'information_office' before that enum value exists.
...
-- Storage news-covers (drop legacy policies that cast invalid enum literals)
